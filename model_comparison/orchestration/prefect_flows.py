...
def prepare_experiment_data(
    ...
    # Prepare data splits in parallel for each (site, seed) combination
    site_seed_data_refs = {}
    site_seed_data_refs_openva = {}
    
    # Collect all (site, seed) combinations needed
    site_seed_combinations = set()
    for exp in experiments:
        site = exp.get("site", exp.get("train_site"))
        seed = exp.get("experiment_metadata", {}).get("random_seed")
        if site and seed is not None:
            site_seed_combinations.add((site, seed))

    # Submit data preparation tasks for both data formats
    prep_tasks = {}
    prep_tasks_openva = {}
    for site, seed in site_seed_combinations:
        key = f"{site}_seed{seed}"
        prep_tasks[key] = prepare_data_for_site.remote(
            data, site, random_seed=seed
        )
        if data_openva is not None:
            prep_tasks_openva[key] = prepare_data_for_site.remote(
                data_openva, site, random_seed=seed
            )

    # Collect results
    for key, task_ref in prep_tasks.items():
        result = ray.get(task_ref)
        if result is not None:
            X_train, X_test, y_train, y_test = result
            site_seed_data_refs[key] = {
                "train_data": ray.put((X_train, y_train)),
                "test_data": ray.put((X_test, y_test)),
            }
    
    # Collect OpenVA results if available
    for key, task_ref in prep_tasks_openva.items():
        result = ray.get(task_ref)
        if result is not None:
            X_train, X_test, y_train, y_test = result
            site_seed_data_refs_openva[key] = {
                "train_data": ray.put((X_train, y_train)),
                "test_data": ray.put((X_test, y_test)),
            }
    ...
    for exp in experiments:
        ...
        # Get seed and model info
        model_name = exp.get("model_name", "")
        seed = exp.get("experiment_metadata", {}).get("random_seed")

        if exp["experiment_type"] == "in_domain":
            site = exp["site"]
            site_seed_key = f"{site}_seed{seed}"
            
            # Choose appropriate data format based on model
            if model_name == "insilico" and site_seed_key in site_seed_data_refs_openva:
                exp_copy["train_data"] = site_seed_data_refs_openva[site_seed_key]["train_data"]
                exp_copy["test_data"] = site_seed_data_refs_openva[site_seed_key]["test_data"]
            elif site_seed_key in site_seed_data_refs:
                exp_copy["train_data"] = site_seed_data_refs[site_seed_key]["train_data"]
                exp_copy["test_data"] = site_seed_data_refs[site_seed_key]["test_data"]
            else:
                continue  # Skip if data not found
                
            # Add OpenVA data for ensemble models
            if model_name == "ensemble" and site_seed_key in site_seed_data_refs_openva:
                exp_copy["experiment_metadata"]["train_data_openva"] = site_seed_data_refs_openva[site_seed_key]["train_data"]
                exp_copy["experiment_metadata"]["test_data_openva"] = site_seed_data_refs_openva[site_seed_key]["test_data"]
            ...
            train_site_seed_key = f"{train_site}_seed{seed}"
            test_site_seed_key = f"{test_site}_seed{seed}"
            
            # Choose appropriate data format based on model
            if model_name == "insilico":
                if train_site_seed_key in site_seed_data_refs_openva and test_site_seed_key in site_seed_data_refs_openva:
                    exp_copy["train_data"] = site_seed_data_refs_openva[train_site_seed_key]["train_data"]
                    exp_copy["test_data"] = site_seed_data_refs_openva[test_site_seed_key]["test_data"]
                else:
                    continue  # Skip if data not found
            else:
                if train_site_seed_key in site_seed_data_refs and test_site_seed_key in site_seed_data_refs:
                    exp_copy["train_data"] = site_seed_data_refs[train_site_seed_key]["train_data"]
                    exp_copy["test_data"] = site_seed_data_refs[test_site_seed_key]["test_data"]
                else:
                    continue  # Skip if data not found
            ...
            if model_name == "ensemble":
                if train_site_seed_key in site_seed_data_refs_openva and test_site_seed_key in site_seed_data_refs_openva:
                    exp_copy["experiment_metadata"]["train_data_openva"] = site_seed_data_refs_openva[train_site_seed_key]["train_data"]
                    exp_copy["experiment_metadata"]["test_data_openva"] = site_seed_data_refs_openva[test_site_seed_key]["test_data"]
